...
#### ciorna
Fisher_scoring_alg<-function(beta_init, X, y, f=my_f, s=my_s, max_iter=1e4, tol=1e-5, k_second=my_k_second, k_prime=my_k_prime, verbose= TRUE)
{ 
  ...
  beta_old<-beta_new
  beta_new<-beta_old + solve(f(X, beta_old, k_second)) %*% s(X,beta_old, y, k_prime)
  #print(solve(f(X, beta_old, k_second)) %*% s(X,beta_old, y, k_prime))
  if (verbose == TRUE)
  {if (k %%2==1)
  {print(beta_old-beta_new[1:5])
   #print("beta")
   #print(beta_new)
    }}
  
  if (sum(abs(beta_old-beta_new)) <= tol )
    {return (beta_new)}
  } 
...
fisher_scoring_lasso <- function(y,X,beta_init, tol, max_iter, interactions, lambda){
  ...
  repeat{
    ...
    beta1 <- optim(par = beta0, fn = total_loss, gr = total_gradient, Z = Z, X = X, W = W, interactions=interactions, lambda=lambda, method ='BFGS',control = list(maxit = 40))$par
