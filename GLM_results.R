...
folder_path <- 'Results'
filename <- 'example_vector.txt'

store_vector_as_txt(vector, column_names, folder_path, filename)
read_vector_from_txt(folder_path, filename)


column_names <- c('A', 'C', 'E')
vector_info <- read_vector_from_txt(folder_path, filename)
values <- get_elements(column_names, vector_info)
print(values)
...
beta_result1<-fisher_scoring(y=y_train, X=X_all_train, beta_init=lm(y_train~X_all_train)$coef,tol=1e-3, max_iter=1e2)$beta.hat

#beta_result1
pred_train<-sapply(as.vector(as.matrix(X_all_train)%*%beta_result1[-1]+beta_result1[1]), my_k_prime)
pred_test<-sapply(as.vector(as.matrix(X_all_test)%*%beta_result1[-1]+beta_result1[1]), my_k_prime)
print("rmse mine:")
...
#######
##USE STANDARD FISHER SCORING 


print(dim(X_all_train))
beta_result1<-Fisher_scoring_alg(y=y_train, X=X_all_train, beta_init=lm(y_train~X_all_train)$coef,tol=1e-2, max_iter=15, lambda=0.5, interactions=list_of_interactions)
length(beta_result1)
...
beta_result<-fisher_scoring_lasso(y=y_train, X=X_all_train, beta_init=lm(y_train~X_all_train)$coef, interactions=list_of_interactions, lambda=100,tol=0.01, max_iter=15) #The higher no of iters the better the result but takes loong
