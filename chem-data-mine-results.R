...
####################Put matrix such that have only 1 and 0. In this way will have 44 columns###############################
...
X_train_cap<-X_train
X_train_cap<-create_all_interactions(X_train_cap,num_categories_list=c(15,4), main_lengths=19)$X
beta_interactions<-create_all_interactions(X_train_cap, num_categories_list = c(15,4), main_lengths=19)$beta_interactions
y_train_cap<-y_train
y_scaled_train<-scale(y_train, scale=FALSE)

X_test_cap<-X_test
X_test_cap<-create_all_interactions(X_test_cap,num_categories_list = c(15,4), main_lengths=19)$X
y_test_cap<-y_test
y_scaled_test<-scale(y_test,scale=FALSE)
...
### USE OPTIM FUNCTION /nlm
# Call optim to minimize the function
opt_result <- optim(par = initial_beta,gr=gradient_cap, fn = cap, X = X_train_cap, y = y_scaled_train, lambda=lambda,beta_interactions = beta_interactions, method="BFGS",control = list(maxit = 100))
#opt_result<-nlm(f = function(beta) cap(beta, X_train_cap, y_scaled_train, beta_interactions, lambda = lambda), p = initial_beta, iterlim = 12)
#opt_result
# Extract the optimal beta
optimal_beta <- opt_result$par
